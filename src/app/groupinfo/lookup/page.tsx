...
type LookupSuccess = {
  lookupType: 'leader_manage' | 'member_edit'
...
export default function BookingLookupPage() {
  ...
  }, [form.classDate, form.classHour, form.classMinute, form.instructorName, hasValidParentPhone])
  ...
  const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (lookupResult.manageToken) {
        router.replace(`/manage/${lookupResult.manageToken}`)
        return
      }

      if (lookupResult.editToken) {
        router.replace(`/member/edit/${lookupResult.editToken}`)
        return
      }

      setError('이동할 수 있는 조회 결과가 없습니다. 다시 확인해 주세요.')
    } catch (_err) {
  ...
  return (
    <main className={styles.page}>
      <section className={styles.panel}>
        ...
        <h1 className={`font-display ${styles.title}`}>예약 내역 조회/수정</h1>
        <p className={styles.description}>예약 당시 입력한 일정, 강사명, 학부모 연락처로 조회/수정 페이지로 이동합니다.</p>
