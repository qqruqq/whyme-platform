  ...
  return (
    <section className={styles.block}>
      ...
      <YesNoField
        ...
        onChange={(next) => onChange({ ...student, priorStudentAttended: next })}
      />
      ...
      <YesNoField
        ...
        onChange={(next) => onChange({ ...student, siblingsPriorAttended: next })}
      />
      ...
      <YesNoField
        ...
        onChange={(next) => onChange({ ...student, parentPriorAttended: next })}
      />
    </section>
  )
}
...
export default function BookingPage() {
  ...
  const hasValidCashReceiptNumber = !hasCashReceiptNumber || (cashReceiptPrefix.length === 3 && cashReceiptSuffix.length === 8)
  const cashReceiptNumber = hasCashReceiptNumber ? composePhoneNumber(cashReceiptPrefix, cashReceiptSuffix) : undefined

  const firstStudentReady = studentComplete(form.firstStudent)
  const secondStudentReady = !form.includeSecondStudent || studentComplete(form.secondStudent)
            ...
            {form.acquisitionChannel === '기타' ? (
              <label className={styles.field}>
                ...
                <input
                  ...
                  onChange={(event) => setForm((prev) => ({ ...prev, acquisitionEtc: event.target.value }))}
